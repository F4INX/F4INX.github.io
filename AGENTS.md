...
redirect output to a log file first, then process the log file. This also saves
time when you need to change the processing command — you can re-run the
grep/sort on the existing log file instead of re-running LinkChecker each time.
