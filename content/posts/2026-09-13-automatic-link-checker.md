...
After some benchmarks, I settled on [LinkChecker](https://github.com/linkchecker/linkchecker), a mature tool written in Python.
...
At the beginning, there were several bash scripts using grep/sed/awk. Then for CSV parsing the bash scripts used inline Python. Then I reworked these to use only a single Python script, available as [`linkchecker.py`](https://github.com/F4INX/F4INX.github.io/blob/master/linkchecker.py) in the repository.

## GitHub action file

The [GitHub action file](https://github.com/F4INX/F4INX.github.io/blob/master/.github/workflows/link-check.yml) allows checking for broken links 1/ when changes are made and 2/ monthly, on the first day of each month, to avoid link rot. Additionally, GitHub sends me an email when this action fails so I'm warned that I have to update links.
...
![Summary page generated by the link checker, showing statistics, redirects, ignored, and cached links.](/posts/automatic-link-checker/summary.webp)
